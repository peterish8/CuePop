...
export const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        "cue-focus flex h-10 w-full items-center justify-between gap-2 rounded-xl border border-[var(--color-border-strong)] bg-white/[.035] px-3.5 text-sm text-[var(--color-foreground)] disabled:opacity-50 [&>span]:line-clamp-1",
        className,
      )}
      {...props}
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDown className="size-4 text-[var(--color-foreground-subtle)]" />
  ...
  ({ className, children, position = "popper", ...props }, ref) => (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        position={position}
        className={cn(
          "cue-panel z-50 max-h-80 min-w-[8rem] overflow-hidden p-1",
...
export const SelectItem = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Item>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item>>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-pointer select-none items-center rounded-lg py-2 pl-8 pr-2 text-sm text-[var(--color-foreground)] outline-none data-[highlighted]:bg-white/[.06] data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className,
      )}
      {...props}
    >
