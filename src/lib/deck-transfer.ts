...
const transferSchema = z.object({
  format: z.literal("cuepop-deck"), version: z.literal(1), exportedAt: z.string().datetime(),
  deck: z.object({
...
export function createDeckTransfer(deck: Deck) {
  return { format: "cuepop-deck" as const, version: 1 as const, exportedAt: new Date().toISOString(), deck: {
