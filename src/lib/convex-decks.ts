...
function client() { const url = process.env.NEXT_PUBLIC_CONVEX_URL; const secret = process.env.CONVEX_SERVER_SECRET; if (!url || !secret) throw new Error("CuePop storage is not configured."); return { live: new ConvexHttpClient(url), secret }; }
