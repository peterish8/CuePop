...
export function RemoteControl({ code }: { code: string }) {
  ...
  return <main className="min-h-svh bg-[var(--color-surface)] px-5 pb-[252px] pt-6 text-white"><div className="mx-auto max-w-md"><header className="flex items-center justify-between"><DeckactiveLogo /><div className="flex items-center gap-2 text-xs text-[var(--color-foreground-subtle)]"><span className={`size-2 rounded-full ${connected ? "bg-[var(--color-success)]" : "bg-[var(--color-warning)]"}`} />{code}</div></header>
